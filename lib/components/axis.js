...
var Axis = React.createClass({displayName: "Axis",
    ...
    generateTicks: function(ticks) {
        ...
        return _.map(ticks, function(tick, i) {
            ...
            return (
                React.createElement("g", {className: 'tick', key: i, transform: this.getTransformForTick(tick)}, 
                    text
                )
            )
        }, this);
    ...
    generateDomain: function(d) {
        ...
        return (React.createElement("path", {d: path, strokeWidth: 1, style: style}));
    },

    getTransform: function() {
        var orientation = this.props.orientation;
        if (orientation === axis_orientations.BOTTOM) {
            return this._formatTranslate(0, this.props.height - this.props.margin.top - this.props.margin.bottom);
    ...
    render: function() {
        ...
        var ticks = scale.ticks ? scale.ticks(this.props.ticks) : scale.domain();
        var domain = [];

        return (
            React.createElement("g", {className: this.props.className, transform: this.getTransform()}, 
